    ...
    while pos_1 < len(s1) and still_ok:
        ...
        while pos_2 < len(a_list) and not found:
            if s1[pos_1] == a_list[pos_2]:
                ...
                pos_2 = pos_2 + 1
        if found:
            # replaces the char so we can detect cases like apppe & apple
            a_list[pos_2] = None
        else:
            still_ok = False
        pos_1 = pos_1 + 1
...
print(anagram_solution_1("apppe", "pleap"))  # expected: False
...
# Optimal solution
# any two anagrams wil have the same number of characters
# O(2n + 26) --> O(n)
...
print(anagram_solution_3("abcd", "dcba"))  # expected: True
